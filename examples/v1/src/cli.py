...
from .operations import convert_ctdb_models_to_nest
from .utils import MVParMethod
...
def get_build_adaptor(nc: NetworkConstructor):
    ...
    from .dm_network import MVDenseNetwork
    match nc:
        case 'DenseNetwork':
            return DenseNetwork
        case "MVDenseNetwork":
...
class CerebrumRecipeRunner(pydantic.BaseModel):
    ...
    build_adaptor_class: NetworkConstructor = 'MVDenseNetwork'
    build_parallel_method: MVParMethod = MVParMethod.ALL_GATHER_BY_SND_RCV
    base_dir: Path = Path("./")
    recipe_dir: Path = Path("./recipes/v1")
    recipe_output_dir: Path | None = Path("./data/v1")
    ...
    custom_mod_main: Path = Path("./recipes/v1/custom_mod.json")
    custom_mod_levels: dict[str, str | Path] = {
        "L1": Path("./recipes/v1/custom_mod_l1.json"),
        "L23": Path("./recipes/v1/custom_mod_l23.json"),
        "L4": Path("./recipes/v1/custom_mod_l4.json"),
        "L5": Path("./recipes/v1/custom_mod_l5.json"),
        "L6": Path("./recipes/v1/custom_mod_l6.json"),
    }
    custom_mod_exts: list[str | Path] = [
        Path("./recipes/v1/custom_mod_ext.json"),
        Path("./recipes/v1/custom_mod_ext_lgn.json"),
        Path("./recipes/v1/custom_mod_ext_bkg.json"),
    ]
    ...
    def output_dir(self) -> Path:
        return Path(
            self.recipe_output_dir
            if self.recipe_output_dir else self.recipe_dir
        )
...
def recipe_bmtk(
    ...
    comm: CommInterface = default_comm()
...
def struct_bmtk(rcp_set: CerebrumRecipeRunner):
    from .comm_interface import default_comm, CommInterface
    comm: CommInterface = default_comm()
    # Dont save it to db when running with MPI
    if comm.size > 0:
    ...
    mdr_build(
        mdrcp, rcp_set.build_adaptor_class, rcp_set.build_parallel_method
    )
    comm.log_profile_summary(
        _log(),
        logging.DEBUG,
...
def main(input_config_file: str):
    ...
    comm: CommInterface = default_comm()
    if comm.rank == 0:
        logging.basicConfig(level=rcp_runner.root_logging_level)
    else:
        logging.basicConfig(level=rcp_runner.logging_level)
    comm.log_at_root(
        _log(), logging.INFO, rcp_runner.model_dump_json(indent=4)
    )
    # rcp_runner.convert_models_to_nest()
    # build_bmtk(rcp_runner)
    if rcp_runner.enable_timing:
        TimerConfig.enable_timers()
    else:
        TimerConfig.disable_timers()
    #
    struct_bmtk(rcp_runner)

...
# raise exception if params across MPI procs are not same.
MPI_fail_params_nonuniform = True
...
class CommInterface:
    ...
    def counts_start_index(self, ncounts: int) -> int:
        return 0
...
try:
    ...
    class MPIComm(CommInterface):
        ...
        def check_properties_across_ranks(
            ...
            """
            Checks that a properties table is consistant across all MPI ranks.
            Mainly used by add_nodes() and add_edges()
            method due to bug where using random generator without rng_seed was
            causing issues building the network properties consistantly using
            multiple cores.

            Will throw an Exception or a warning message
            (if MPI_fail_params_nonuniform is false)
            ...
            # For each item in model_properties dictionary try to check that
            #   values are the same
            for pkey in prop_keys:
                # Don't use Dict.items() method since it is possible
                #  the ret order is different across ranks.
                ...
                try:
                    ...
                    # If the hashing fails assume there is no MPI data issue 

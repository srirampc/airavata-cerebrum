...
def identify_cell_type(px: str):
    return "Exc" if px[0] == 'e' else ast.literal_eval(px[1:])[1]
...
def determine_sort_position(v1df, sortby):
    if v1df["location"].iloc[0] == "Cortex":  # Old model
        layer = v1df["pop_name"].apply(lambda x: ast.literal_eval(x)[0])
        v1df["location"] = layer
...
def make_figure_elements(config_file, radius, sortby):
    ...
    v1df["Cell Type"] = (v1df["ei"] + v1df["pop_name"]).apply(identify_cell_type)
